...
def main():
    ...
    with rate_limiter:
        for (project, num_bindings) in _find_resources(
            projects_api, folders_api, **filter_params):
...
def _find_resources(projects_api, folders_api, **filter_params):
    ...
    """
    proj_request = projects_api.list(filter=
        _build_filter(filter_params))
    ...
    for f in folder_res.get('folders', []):
        ...
        yield _find_resources(projects_api, folders_api, **folder_filter)
